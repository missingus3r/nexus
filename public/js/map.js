...
function createIncidentPopupHTML(incident) {
    ...
    // News sources section (for auto-verified incidents)
    let newsSourcesHTML = '';
    if (props.autoGenerated && props.sourceNews && props.sourceNews.length > 0) {
        newsSourcesHTML = `
            <div style="margin: 10px 0; padding: 10px; background: #e3f2fd; border-left: 3px solid #2196f3; border-radius: 4px;">
                <div style="font-weight: 600; color: #1976d2; margin-bottom: 8px; display: flex; align-items: center; gap: 6px;">
                    <span>📰</span>
    ...
    if (isAuthenticated && status === 'pending') {
        ...
        actionsHTML = `
            <div style="margin-top: 10px; padding: 8px; background: #e3f2fd; border-radius: 4px; font-size: 0.9em; text-align: center;">
                ✅ Incidente verificado automáticamente desde fuentes de noticias confiables
            </div>
        `;
